...
function render(project: ProjectProps, tasks: readonly TaskProps[]): string {
  ...
  const rows = groups.length ? groups.map(group => `<section class="group" data-group="${group.id}"><div class="group-title"><h2>${escapeHtml(group.label)}</h2><span>${group.tasks.length}</span></div><div class="tasks">${group.tasks.map(task => `<button class="task" data-task="${escapeHtml(task.id)}" data-search="${escapeHtml(`${task.title} ${task.status} ${task.priority}`.toLowerCase())}"><span><strong>${escapeHtml(task.title)}</strong><small>${escapeHtml(taskPriorityLabel(task.priority))} priority · updated ${escapeHtml(task.updatedAt.toLocaleString())}</small></span><span class="status">${escapeHtml(taskStatusLabel(task.status))}</span></button>`).join("")}</div></section>`).join("") : '<p class="empty">No tasks in this project yet.</p>';
...
body{font-family:var(--vscode-font-family);color:var(--vscode-foreground);padding:24px;max-width:1050px;margin:auto}h1{margin-bottom:4px}.path,.updated{color:var(--vscode-descriptionForeground);overflow-wrap:anywhere}.updated{margin-top:6px}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin:24px 0}.metric{border:0;text-align:left;color:var(--vscode-foreground);background:var(--vscode-editor-inactiveSelectionBackground);padding:16px;border-radius:5px}.metric[data-filter]{cursor:pointer}.metric[data-filter]:hover,.metric.selected{outline:1px solid var(--vscode-focusBorder)}.metric strong{display:block;font-size:24px}.toolbar{display:flex;gap:8px;margin:0 0 20px}.toolbar input{flex:1;min-width:120px;padding:7px 9px;color:var(--vscode-input-foreground);background:var(--vscode-input-background);border:1px solid var(--vscode-input-border)}.toolbar button{color:var(--vscode-button-foreground);background:var(--vscode-button-background);border:0;padding:7px 12px;cursor:pointer}.group{margin:0 0 24px}.group-title{display:flex;align-items:center;gap:8px}.group-title h2{font-size:16px}.group-title span{color:var(--vscode-badge-foreground);background:var(--vscode-badge-background);padding:1px 7px;border-radius:10px}.tasks{display:grid;gap:8px}.task{display:flex;justify-content:space-between;align-items:center;text-align:left;border:1px solid var(--vscode-panel-border);background:var(--vscode-list-inactiveSelectionBackground);color:var(--vscode-foreground);padding:12px 14px;cursor:pointer}.task:hover{background:var(--vscode-list-hoverBackground)}.task span:first-child{display:grid;gap:4px}.task small{color:var(--vscode-descriptionForeground)}.status{background:var(--vscode-badge-background);color:var(--vscode-badge-foreground);padding:3px 9px;border-radius:10px}.empty{color:var(--vscode-descriptionForeground)}[hidden]{display:none!important}</style></head><body>
<h1>${escapeHtml(project.name)}</h1><div class="path">${escapeHtml(project.repositoryRoot)} · base ${escapeHtml(project.baseRef)}</div><div class="updated">${latest ? `Last task update: ${escapeHtml(latest.toLocaleString())}` : "No task activity yet"}</div><section class="metrics"><button class="metric selected" data-filter="all"><strong>${tasks.length}</strong>Total tasks</button><button class="metric" data-filter="active"><strong>${active}</strong>Active</button><button class="metric" data-filter="attention"><strong>${attention}</strong>Needs attention</button><button class="metric" data-filter="completed"><strong>${completed}</strong>Completed</button></section><div class="toolbar"><input id="search" type="search" placeholder="Search tasks by title, status, or priority"><button id="clear">Clear</button></div><main>${rows}</main><script nonce="${nonce}">const vscode=acquireVsCodeApi();let filter='all';const search=document.getElementById('search');const apply=()=>{const query=search.value.trim().toLowerCase();document.querySelectorAll('.group').forEach(group=>{let visible=0;group.querySelectorAll('[data-task]').forEach(row=>{const matchesFilter=filter==='all'||group.dataset.group===filter||(filter==='completed'&&group.dataset.group==='completed');const show=matchesFilter&&row.dataset.search.includes(query);row.hidden=!show;if(show)visible++});group.hidden=visible===0});};document.querySelectorAll('[data-task]').forEach(row=>row.addEventListener('click',()=>vscode.postMessage({type:'openTask',taskId:row.dataset.task})));document.querySelectorAll('[data-filter]').forEach(button=>button.addEventListener('click',()=>{filter=button.dataset.filter;document.querySelectorAll('[data-filter]').forEach(item=>item.classList.toggle('selected',item===button));apply()}));search.addEventListener('input',apply);document.getElementById('clear').addEventListener('click',()=>{search.value='';filter='all';document.querySelectorAll('[data-filter]').forEach(item=>item.classList.toggle('selected',item.dataset.filter==='all'));apply()});</script></body></html>`;
}
