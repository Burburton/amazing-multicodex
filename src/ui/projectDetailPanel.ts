...
function render(project: ProjectProps, tasks: readonly TaskProps[]): string {
  ...
  const health = projectMetrics(tasks);
  const average = health.averageStageDurationMs === undefined ? "n/a" : `${Math.round(health.averageStageDurationMs / 1_000)}s`;
  const groups = [{ id: "metrics" as const, label: `Health · ${health.failedStages} failed stages · ${health.runningAgents} running agents · avg stage ${average}`, tasks: [] as readonly TaskProps[] }, ...groupProjectTasks(tasks)];
