...
function render(detail: TaskDetailViewModel): string {
  ...
  const execution = detail.latestExecution
    ? `<dl><dt>Status</dt><dd>${escapeHtml(detail.latestExecution.status)}</dd>${detail.latestExecution.stage ? `<dt>Agent stage</dt><dd>${escapeHtml(roleLabel(detail.latestExecution.stage.role))} · ${detail.latestExecution.stage.index + 1} of ${detail.latestExecution.stage.total}</dd><dt>Retained prior sessions</dt><dd>${detail.latestExecution.previousAgents?.length ?? 0}</dd><dt>Review cycles</dt><dd>${detail.latestExecution.reviewCycles ?? 0} of 3</dd>` : ""}<dt>Branch</dt><dd><code>${escapeHtml(detail.latestExecution.workspace.branch)}</code></dd><dt>Worktree</dt><dd><code>${escapeHtml(detail.latestExecution.workspace.path)}</code></dd></dl>`
